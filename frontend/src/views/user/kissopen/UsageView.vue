  ...
  <AppLayout>
    <div class="kact-head">
      ...
      <div class="kact-head__actions">
        <button class="ko-iconbtn ko-iconbtn--outline" :aria-label="t('usage.timeRange')" type="button">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 4h18l-7 8v6l-4 2v-8L3 4z" /></svg>
        </button>
    ...
    <div v-if="pageView === 'trends'" class="kact-pane"><KitActivityTrends :start-date="filters.start_date || startDate" :end-date="filters.end_date || endDate" /></div>
    <div v-else-if="pageView === 'explore'" class="kact-pane">
      <KitUsageExplore :start-date="filters.start_date || startDate" :end-date="filters.end_date || endDate" />
    </div>
    <div v-else-if="pageView === 'guardrails'" class="kact-pane"><KitActivityGuardrails /></div>
    <TablePageLayout v-show="pageView === 'overview'">
      <template #actions>
        <KitActivityOverview :data="realOverview" @explore="pageView = 'explore'" />
      </template>
...
import type { UsageLog, ApiKey, UsageQueryParams, UsageStatsResponse, UserErrorRequest, TrendDataPoint, ModelStat } from '@/types'
...
// --- kissopen Activity overview (real data) ---
...
// Build the shared Activity-overview data shape (same as the design demo) from
// real API data: stat sparklines + by-day charts from the trend, by-model bars
// from model stats, and top keys / providers from the aggregate endpoint.
...
const loadOverview = async () => {
  try {
    const s = filters.value.start_date || startDate.value
    const e = filters.value.end_date || endDate.value
    const [trend, models, keysAgg, provAgg] = await Promise.all([
      usageAPI.getDashboardTrend({ start_date: s, end_date: e, granularity: 'day' }),
      usageAPI.getDashboardModels({ start_date: s, end_date: e }),
      usageAPI.getUsageAggregate({ dimension: 'api_key', start_date: s, end_date: e }),
      usageAPI.getUsageAggregate({ dimension: 'provider', start_date: s, end_date: e }),
    ])
...
// Header time-range select (design parity): map a preset to start/end dates,
// keep the table's DateRangePicker in sync, and reload everything.
const applyRange = () => {
...
<style scoped>
/* kissopen: unified soft-coral stat chip (replaces the multicolor icon chips). */
.ko-chip {
  display: inline-flex;
  align-items: center;
